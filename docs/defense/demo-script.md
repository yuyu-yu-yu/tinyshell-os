...
目标时长 6 分钟。第一轮预演可看稿；第二轮合稿后随机抽问。第四轮已接入 Ring 0 Shell，现场走 `tiny> ` 固定命令，并口头强调不是用户态。
...
动作：在 QEMU 里按固定顺序操作，不要即兴加命令。口播：IRQ1 只入队，命令在前台跑；Shell 仍在 Ring 0。
...
| Shell 无提示符 | 展示 `qemu-shell-serial.log` 与 `BOOT_OK`/`SHELL_READY`，承认现场 QEMU 问题 |
